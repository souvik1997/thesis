...
Blockchain and cryptocurrency are two different concepts, but their development has been intertwined. Blockchain was first suggested by Satoshi Nakamoto's paper introducing Bitcoin in late 2008. Bitcoin is a cryptocurrency and is a particular implementation of blockchain technology.
...
\begin{enumerate}
  ...
  \begin{align*}
    ...
    \text{$G_{memory}$ is a constant defined to be $G_{memory} = 3$}
  \end{align*}
...
\begin{protocol}{Ethereum block synchronization protocol}
  ...
  \begin{enumerate}
    ...
    \item $P_1$ replies with a \textsc{BlockHeaders} message, containing a list of block headers. \label{syncprotocol:replyBlockHeaders}
    ...
    \item $P_1$ replies with \textsc{BlockBodies} containing the bodies of blocks $P_2$ requested. \label{syncprotocol:replyBlockBodies}
